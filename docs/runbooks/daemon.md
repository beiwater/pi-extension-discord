...
- `telegram.config.ts`（项目根，唯一配置文件）：复制 `telegram.config.example.ts` 后编辑，`defineConfig()` 提供类型与逐字段注释。它会作为受信本机代码执行，不要粘贴来源不明的配置。
  ...
  - `context_image_budget_bytes` 默认 10000000，以 active session 实际图片文件字节衡量压力，超限时多触发一次普通 compaction。compaction 保留窗口按每张图片约 1,100 token 计入 `compaction_keep_recent`（Pi 自身的估算对上下文图片计 0），所以 20,000 的保留预算最多留下约 15 张图片；CJK 文本仍按 Pi 的 chars/4 估算，真实 token 约为其 3 倍。共享文件仅由 media lifecycle 按所有 bot 的引用回收，不保证单次压缩一定回收全部图片。
