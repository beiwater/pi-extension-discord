...
- `telegram.config.ts`（项目根，唯一配置文件）：复制 `telegram.config.example.ts` 后编辑，`defineConfig()` 提供类型与逐字段注释。它会作为受信本机代码执行，不要粘贴来源不明的配置。
  ...
  - `context_image_budget_bytes` 默认 10000000，以 active session 实际图片文件字节衡量压力，超限时多触发一次普通 compaction。在 Pi 准备压缩之前，保留窗口按每张图片约 1,100 token 计入 `compaction_keep_recent`；切点仍由 Pi 按完整 entry / turn 决定，因此预算是估值而非严格图片数量上限。摘要模型支持图片输入时，同一次摘要请求会携带待摘要图片；不支持或文件缺失时保留文字占位并记录退化计数，估计输入超过摘要模型窗口时取消压缩并保留原上下文。共享文件仅由 media lifecycle 按所有 bot 的引用回收，不保证单次压缩一定回收全部图片。
